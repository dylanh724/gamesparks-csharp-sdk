...
namespace SuperSocket.ClientEngine.Proxy
{
	public class HttpConnectProxy : ProxyConnectorBase
	{
		...
		{
			GameSparks.Core.GameSparksUtil.Log("Connect1");
		...
		protected override void ProcessConnect(Socket socket, object targetEndPoint, SocketAsyncEventArgs e)
		{
			...
			{
				var targetDnsEndPoint = (DnsEndPoint2)targetEndPoint;
#endif
			...
			var requestData = System.Text.Encoding.UTF8.GetBytes(request); //UTF8Encoding.GetBytes(request);

			GameSparks.Core.GameSparksUtil.Log(request);
		...
		protected override void ProcessReceive(SocketAsyncEventArgs e)
		{
			...
			var lineReader = new StringReader(Encoding.UTF8.GetString(e.Buffer, 0, responseLength));
			...
			//if (!m_ResponsePrefix.Equals(httpProtocol) && !m_ResponsePrefix2.Equals(httpProtocol))
			//{
